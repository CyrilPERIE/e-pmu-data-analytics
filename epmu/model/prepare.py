"""
TODO:
- add nebulosite number feature
- scrap "musique" feature
"""
...
def get_all_feature_names() -> List[str]:
    ...
    feature_names = [
        ...
        "prob_std",
        "prob_range",
        "prob_favorite_second_diff",
        "prob_favorite_second_ratio",
        "prob_favorite_mean_ratio",
...
def extract_time_features(timestamp: int) -> Dict[str, float]:
    ...
    return {"hour": dt.hour, "minute": dt.minute, "is_afternoon": 1 if dt.hour >= 12 else 0}
...
def extract_features(race_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    # Extract nebulosite number
    nebulosite = meteo.get("nebulosite", "UNKNOWN")
    if nebulosite != "UNKNOWN":
        # Extract the number from the nebulosite code (e.g., "P10" -> 10)
        match = re.search(r"P(\d+)", nebulosite)
        features["nebulosite_number"] = float(match.group(1)) if match else 0
    else:
        features["nebulosite_number"] = 0
    ...
    if len(sorted_odds) >= 2:
        ...
        features["prob_favorite_mean_ratio"] = (
            probabilities[0] / np.mean(probabilities) if np.mean(probabilities) > 0 else 0
        )
    else:
    ...
    if betting_amounts:
        ...
        if len(sorted_amounts) >= 2:
            features["top_2_bet_ratio"] = (
                sum(sorted_amounts[:2]) / sum(betting_amounts) if sum(betting_amounts) != 0 else 0
            )
        else:
...
def prepare_dataset() -> Tuple[pd.DataFrame, pd.Series, np.ndarray]:
    """
    Prepare X (features), y (target), and winning_odds for the model.
    Returns:
        X: Features DataFrame
        y: Target Series (1 if favorite won, 0 otherwise)
        winning_odds: Array of odds for winning horses
    """
    all_features = []
    targets = []
    winning_odds = []
    ...
    for file in sorted(DATA_DIR.glob("*.json")):
        with open(file, "r") as f:
            ...
            for race_key, race_data in day_data.items():
                ...
                # Get winning odds
                if target == 1:  # If favorite won
                    odds = race_data["rapportsDefinitifs"]["E_SIMPLE_GAGNANT"].get(winner, 2.5)
                else:
                    odds = 0  # No payout if lost
    ...
    y = pd.Series(targets)
    winning_odds = np.array(winning_odds)
    ...
    np.save("epmu/model/data/winning_odds.npy", winning_odds)
    ...
    print(f"Mean winning odds: {np.mean(winning_odds[winning_odds > 0]):.2f}")
    print(f"Median winning odds: {np.median(winning_odds[winning_odds > 0]):.2f}")

    return X, y, winning_odds
